...
import {
  el, toast, confirmSheet, emptyState, debounce, listItem,
} from '../ui.js';
...
function listView() {
  ...
  function paint() {
    ...
    if (favs.length && !query.trim()) {
      ...
      for (const ex of favs.sort((a, b) => a.name.localeCompare(b.name))) {
        list.append(listItem({
          title: ex.name,
          sub: `${ex.muscle} · ${ex.equipment}`,
          ariaLabel: `Open ${ex.name}`,
    ...
    const ordered = [...pool].sort(
      (a, b) => MUSCLES.indexOf(a.muscle) - MUSCLES.indexOf(b.muscle) || a.name.localeCompare(b.name));
    ...
    for (const ex of shown) {
      ...
      list.append(listItem({
        ...
        sub: `${ex.equipment}${uses ? ` · ${uses} ${uses === 1 ? 'session' : 'sessions'}` : ''}${ex.isCustom ? ' · custom' : ''}`,
        ariaLabel: `Open ${ex.name}`,
        onclick: () => navigate('library', ex.id),
      }));
    }
  ...
  pane.append(
    ...
    chips,
    el('div', { style: { marginBottom: '12px' } }, [equipSel]),
    list,
    footer
  );
  return pane;
  ...
  root.append(
    el('div.stack', { style: { marginTop: '18px' } }, [
      ...
      el('button.btn.full.danger', {
        onclick: async () => {
          const warn = uses
            ? `${ex.name} appears in ${uses} logged ${uses === 1 ? 'session' : 'sessions'}. Those sessions keep their sets, but the exercise name will be lost from them.`
            : `${ex.name} will be removed from your library and any plans using it.`;
